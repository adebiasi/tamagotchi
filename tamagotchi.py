...
OVERLAY_ZZZ = ((0x0, 0x0, 0x0, 0x0, 0xf800000, 0x4000000, 0x2000000, 0x1000000, 0xf800000, 0x0, 0x0, 0x3c00000,
                ...
                0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0), (
                   ...
                   0x800000, 0x3c00000, 0x0, 0x700000, 0x200000, 0x700000, 0x0, 0x80000, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                   0x0,
                   0x0, 0x0, 0x0, 0x0, 0x0))
...
OVERLAY_DEAD = ((
                    0x0, 0x0, 0xfc00000, 0x1fe00000, 0x1b600000, 0x1fe00000, 0xfc00000, 0xfc00000, 0x5400000, 0x0, 0x0,
                    0x0,
                    0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0),
                (
                    ...
                    0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0))
OVERLAY_EXCLAIM = ((
                       0x0, 0x20, 0x70, 0x70, 0x70, 0x70, 0x70, 0x70, 0x70, 0x20, 0x0, 0x20, 0x70, 0x20, 0x0, 0x0, 0x0,
                       0x0,
...
def trigger_clean():
    print("trigger_clean")
    return 0, OVERLAY_CLEAN, True, True


def trigger_eat():
    print("trigger_eat")
    return 0, OVERLAY_EAT, True, True
...
def main():
    ...
    while True:
        ...
        # Event handler
        for _ in pygame.event.get(UPDATE_GAME_EVENT):
            if cleaning_state:
                pygame.time.set_timer(UPDATE_GAME_EVENT, int(SECOND / 10))
            update_game = True
        ...
        if stage > 0:
            if curr_command == COMMANDS.EAT:
                overlay_frame, overlay_anim, eating_state, has_overlay = trigger_eat()
            elif curr_command == COMMANDS.CLEAN:
                overlay_frame, overlay_anim, cleaning_state, has_overlay = trigger_clean()
            elif curr_command == COMMANDS.SLEEP:
                if pet['energy'] <= ENERGY_CANSLEEP:
                    current_anim, overlay_anim, sleeping_state, has_overlay = trigger_sleep(stage)

        # Game logic
        if update_game:
            print("has overlay before evo: " + str(has_overlay))
            current_anim, has_overlay, stage = handle_evolution(current_anim, has_overlay, pet, stage)
            print("has overlay after evo: " + str(has_overlay))
            if eating_state and overlay_frame == len(overlay_anim) - 1:
                print("eat finish")
                eating_state = False
                has_overlay = False
                overlay_frame = 0
                ...
                if anim_offset == -33:
                    ...
                    has_overlay = False
                    pet['waste'] = 0
                    pygame.time.set_timer(UPDATE_GAME_EVENT, SECOND)
            else:
                if not dead_state:
                    frame = get_next_frame(current_anim, frame)
                    anim_offset = get_random_offset()
                if not sleeping_state and not dead_state:
                    print("do_cycle")
                    do_cycle(pet)
            ...
            if not sleeping_state and not cleaning_state and not eating_state and not dead_state:
                if pet['waste'] >= WASTE_EXPUNGE:
                ...
                if not dead_state:
                    if pet['hunger'] >= HUNGER_DEADFROMNOTEATING or pet['age'] >= AGE_DEATHFROMNATURALCAUSES:
                        anim_offset = 3
                        current_anim, overlay_anim, dead_state, has_overlay = trigger_death(stage)
            if has_overlay:
                print("has overlay: " + str(overlay_anim))
                overlay_frame = get_next_frame(overlay_anim, overlay_frame)
            update_game = False

        g_interface.render(current_anim, frame, has_overlay, anim_offset, overlay_frame, overlay_anim, pet,
                           selid)


def handle_evolution(current_anim, has_overlay, pet, stage):
    ...
    if stage == 1 and pet['age'] > AGE_MATURE:
        stage += 1
        current_anim = IDLE_MATURE
    return current_anim, has_overlay, stage
